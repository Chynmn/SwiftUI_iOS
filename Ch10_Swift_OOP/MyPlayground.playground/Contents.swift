...
/*  불투명 반환 타입(opaque return type)
 함수 선언부에 결과의 타입을 구체화된 타입(concrete type)으로 지정하는 대신, 불투명 반환 타입을 사용하면 지정된 프로토콜을 따르는 모든 타입이 반환될 수 있게 한다.
 불투명 반환 타입은 프로토콜 이름 앞에 some 키워드를 붙여 선언한다.
 
 밑의 예제는 doubleFunc1()함수가 Equatable 프로토콜을 따르는 모든 타입의 결과가 반환된다고 선언한 것.
 스위프트 표준 프로토콜인 Equatable 프로토콜을 따르기 위해서는 값들이 서로 동일한지 비교할 수 있어야 하지만,
 불투명 반환 타입은 새로 만든 프로토콜을 포함하여 모든 프로토콜에 사용될 수 있다.
 */
func doubleFunc1(value: Int) -> some Equatable {
    value * 2
}

// Int와 String 타입 모두가 Equtable 프로토콜을 따른다면 문자열 결과를 반환하는 함수 또한 생성할 수 있다.
func doubleFunc2(value: String) -> some Equatable {
    value + value
}
/*
 두 메서드는 완전히 서로 다른 구체화된 타입을 반환하지만, 하나 확실한 것은 Equtable 프로토콜을 따른다는 것.
 따라서 우리는 실제 반환 타입을 아는 게 아니라 반환 타입의 자격에 대해 아는 것.
 
 만약 함수가 라이브러리나 API 프레임워크 내에 있어서 소스 코드를 볼 수 없다면, 정확히 어떤 타입이 반환되는지 모를 것.
 공개 API내에서 사용되는 반환 타입을 숨기기 위해 의도적으로 이렇게 설계된다.
 구체화된 타입을 감춤으로써 장점 :
 1. 프로그래머는 특정 구체화된 타입을 반환하는 함수에 의존하지 않게 되거나 접근되지 않는 내부 객체에 접근하게 되는 위험이 없어진다.
 2. API 개발자가 다른 프로토콜과 호환되는 타입을 반환하도록 하는 변경을 포함하여 내부 구현체를 변경할 때 API를 사용하는 모든 코드의 의존성이 깨질까라는 염려를 하지 않아도 된다.
 
 불투명 반환 타입으로 작업할 때 올바르지 않은 추정으로 문제가 발생하지 않을까라는 의문
 ...
 불투명 반환 타입은 SwiftUI API의 기본 토대이며, SwiftUI로 앱을 개발할 때 널리 사용되며 몇몇 키워드는 SwiftUI View 선언 시 자주 등장한다.
 SwiftUI는 작고 재사용 가능한 빌딩 블록을 모으고 거대한 뷰 선언부를 작고 가벼운 하위 뷰들로 리팩토링하여 앱을 만들도록 한다.
 각각의 빌딩 블록은 일반적으로 View 프로토콜을 따른다. 빌딩 블록을 View 프로토콜을 따르는 불투명 타입을 반환하도록 선언함으로써 매우 유연하며 상호호환성 있는
 빌딩 블록이 되며, 그 결과로 더 깔끔하고 재사용과 유지보수하기 쉬운 코드가 된다.
 */
